...
fn sections_bodies<'a>(bytes: &'a [u8]) -> Vec<&'a [u8]> {
    let Ok(mut m) = WasmModule::parse(bytes) else { return Vec::new() };
    m.ensure_function_bodies_parsed();
    let data = m.data();
    m.function_bodies().iter().map(|b| b.body.slice(data)).collect()
}
...
fn canonical_body(body: &[u8]) -> Option<Vec<u8>> {
    use wilt::leb128;
    use wilt::opcode::{self as opc, InstrIter};
    ...
    while let Some((p, _)) = iter.next() {
        ...
        if matches!(op, 0x20 | 0x21 | 0x22) {
            if let Some((idx, _)) = leb128::read_u32(&body[p + 1..]) {
                map.entry(idx).or_insert_with(|| { let i = next_id; next_id += 1; i });
            }
        }
    }
...
fn dupe_stats(bytes: &[u8]) -> (usize, usize, usize) {
    ...
    for b in &bodies { *raw_dupes.entry(b).or_insert(0) += 1; }
...
fn count_callsites(bytes: &[u8]) -> HashMap<u32, u32> {
    use wilt::leb128;
    use wilt::opcode::{self as opc, InstrIter};
    let Ok(mut m) = WasmModule::parse(bytes) else { return HashMap::new() };
    m.ensure_function_bodies_parsed();
    let data = m.data();
    let mut counts: HashMap<u32, u32> = HashMap::new();
    for body in m.function_bodies() {
        let b = body.body.slice(data);
        let Some(start) = opc::skip_locals(b) else { continue };
...
fn inline_opportunity() {
    let Ok(bytes) = std::fs::read("/tmp/real.wasm") else { return };
    ...
    for (i, n) in bucket.iter().enumerate() {
        let label = if i == 5 { ">=5".to_string() } else { i.to_string() };
...
fn dupe_detect() {
    ...
    let wilt_out = wilt::optimise(&bytes);
    let Some(opt_out) = wasm_opt(path) else { return };
    ...
    println!("{:<14} {:>7} {:>12} {:>18}", "source", "funcs", "byte-dupes", "canon-dupes");
    println!("{:<14} {:>7} {:>12} {:>18}", "input", n_in, raw_in, canon_in);
    println!("{:<14} {:>7} {:>12} {:>18}", "wilt", n_w, raw_w, canon_w);
    println!("{:<14} {:>7} {:>12} {:>18}", "wasm-opt", n_o, raw_o, canon_o);
    println!();
