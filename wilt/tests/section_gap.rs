        ...
        for e in entries.flatten() {
            ...
            if p.is_dir() { walk(&p, out); }
                ...
                if let Ok(b) = std::fs::read(&p) {
                    if WasmModule::parse(&b).is_ok() { out.push((p, b)); }
                }
            }
        }
    }
...
fn section_sizes(bytes: &[u8]) -> BTreeMap<u8, usize> {
    ...
    let Ok(m) = WasmModule::parse(bytes) else { return out };
...
fn per_section_gap() {
    ...
    for (_, bytes) in &files {
        ...
        let Some(wo_out) = run_wasm_opt(bytes) else { continue };
        if WasmModule::parse(&wilt_out).is_err() { continue }
        if WasmModule::parse(&wo_out).is_err() { continue }
        for (id, sz) in section_sizes(&wilt_out) { *wilt_sizes.entry(id).or_insert(0) += sz; }
        for (id, sz) in section_sizes(&wo_out) { *wo_sizes.entry(id).or_insert(0) += sz; }
        counted += 1;
    }
    println!("\n── per-section byte totals across {counted} modules ──");
    println!("{:<14} {:>12} {:>12} {:>12}", "section", "wilt", "wasm-opt", "gap");
