...
fn run_wasm_opt(input_path: &str) -> Option<(Vec<u8>, u128)> {
    ...
    let ok = Command::new("wasm-opt")
        .arg("-O").arg(input_path).arg("-o").arg(&out)
...
fn real_binary() {
    ...
    if !validates(&bytes) { println!("/tmp/real.wasm does not validate"); return; }
    ...
    let Some((opt_out, opt_ms)) = run_wasm_opt(path) else {
        println!("wasm-opt failed on real binary"); return;
    };

    println!("\n── {path} — real compiled binary ──");
    println!("input:           {} bytes (gz {})", bytes.len(), gz(&bytes));
    println!("wilt:            {} bytes (gz {})  [{wilt_ms} ms]", wilt_out.len(), gz(&wilt_out));
    println!("wilt_stripped:   {} bytes (gz {})", wilt_strip.len(), gz(&wilt_strip));
    println!("wasm-opt -O:     {} bytes (gz {})  [{opt_ms} ms]", opt_out.len(), gz(&opt_out));
    ...
    println!();
    println!("wilt saved:           {saved_wilt} bytes ({:.1}%)",
        100.0 * saved_wilt as f64 / bytes.len() as f64);
    println!("wilt_stripped saved:  {saved_strip} bytes ({:.1}%)",
        100.0 * saved_strip as f64 / bytes.len() as f64);
    println!("wasm-opt saved:       {saved_opt} bytes ({:.1}%)",
        100.0 * saved_opt as f64 / bytes.len() as f64);
    if saved_opt > 0 {
        println!();
        println!("wilt/wasm-opt ratio:          {:.1}%",
            100.0 * saved_wilt as f64 / saved_opt as f64);
        println!("wilt_stripped/wasm-opt ratio: {:.1}%",
