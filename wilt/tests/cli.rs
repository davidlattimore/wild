...
fn cli_default_output_path() {
    let bin = bin();
    if !bin.exists() { eprintln!("skip: build `cargo build --bin wilt` first"); return; }
    ...
    let expected_out = tmp.with_file_name(
        format!("{}.opt.wasm", tmp.file_stem().unwrap().to_str().unwrap())
    );
    assert!(expected_out.exists(), "default output path {:?} not written", expected_out);
...
fn cli_print_to_stdout() {
    ...
    std::fs::write(&tmp, build_tiny_wasm()).unwrap();
    let output = Command::new(&bin).arg(&tmp).arg("--print").output().unwrap();
    assert!(output.status.success());
    assert!(output.stdout.starts_with(b"\0asm\x01\x00\x00\x00"),
...
fn cli_unknown_flag_exits_2() {
    ...
    let status = Command::new(&bin).arg(&tmp).arg("--zzz-bogus").status().unwrap();
