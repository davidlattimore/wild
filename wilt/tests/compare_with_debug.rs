...
//!
//! Ignored by default. cargo test -p wilt --release --test compare_with_debug -- --ignored --nocapture
...
fn collect(root: &std::path::Path) -> Vec<PathBuf> {
    fn w(d: &std::path::Path, o: &mut Vec<PathBuf>) {
        ...
        for e in es.flatten() {
            let p = e.path();
            if p.is_dir() { w(&p, o); }
            else if p.extension().and_then(|s| s.to_str()) == Some("wasm") { o.push(p); }
        }
    }
    let mut v = Vec::new(); w(root, &mut v); v
}

fn valid(b: &[u8]) -> bool { Validator::new().validate_all(b).is_ok() }

fn wasm_opt(p: &std::path::Path, keep_debug: bool) -> Option<Vec<u8>> {
    let out = std::env::temp_dir().join(format!(
        "wilt_dbg_{}_{}.wasm", std::process::id(),
        p.file_stem().and_then(|s| s.to_str()).unwrap_or("x"),
    ));
    let mut cmd = Command::new("wasm-opt");
    cmd.arg("-O");
    if keep_debug { cmd.arg("-g"); }
    cmd.arg(p).arg("-o").arg(&out)
...
fn compare_both_preserve_debug() {
    ...
    for p in &files {
        ...
        if !valid(&wilt_out) { continue; }
        let Some(o_plain) = wasm_opt(p, false) else { continue };
        let Some(o_keep) = wasm_opt(p, true) else { continue };
    ...
    println!("wilt:                    {wilt_total} (saved {saved_wilt}, {:.1}%)",
        100.0 * saved_wilt as f64 / in_total as f64);
    println!("wasm-opt -O (drops name):{opt_plain} (saved {saved_plain}, {:.1}%)",
        100.0 * saved_plain as f64 / in_total as f64);
    println!("wasm-opt -O -g (keeps):  {opt_keepg} (saved {saved_keep}, {:.1}%)",
        100.0 * saved_keep as f64 / in_total as f64);
    println!();
    if saved_plain > 0 {
        println!("wilt / wasm-opt (-O):    {:.1}%",
            100.0 * saved_wilt as f64 / saved_plain as f64);
    }
    if saved_keep > 0 {
        println!("wilt / wasm-opt (-O -g): {:.1}%",
