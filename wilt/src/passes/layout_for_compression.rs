...
fn any_func_type_uses_non_numeric_valtypes(module: &WasmModule<'_>) -> bool {
    use crate::leb128;
    let Some(sec) = module.section(crate::module::SECTION_TYPE) else { return false };
    let p = sec.payload.slice(module.data());
    let Some((count, mut off)) = leb128::read_u32(p) else { return true };
    for _ in 0..count {
        let Some(&form) = p.get(off) else { return true };
        if form != 0x60 { return true; }     // non-func type: bail
        off += 1;
        for _section in 0..2 {                // params, then results
            let Some((n, c)) = leb128::read_u32(p.get(off..).unwrap_or(&[])) else { return true };
            off += c;
            for _ in 0..n {
                let Some(&vt) = p.get(off) else { return true };
                off += 1;
                if !matches!(vt, 0x7B..=0x7F) { return true; }
            }
        }
    }
    false
...
fn any_body_has_advanced_features(module: &WasmModule<'_>) -> bool {
    ...
    for body in module.function_bodies() {
        ...
        let Some(start) = opcode::skip_locals(b) else { return true };
