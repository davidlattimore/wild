...
pub enum RelaxationKind {
    ...
    /// Replace GOT addressing with direct addressing.
    GotToDirectAddressing,

    /// Replace PC-relative addressing with direct addressing.  
    PcrelToDirectAddressing,

    /// Replace call instruction with JAL instruction.
    CallToJal,

    /// Replace TLS_GD instruction with TLS_IE instruction.
    TlsGdToIe,
}
    ...
    pub fn apply(self, section_bytes: &mut [u8], offset_in_section: &mut u64, _addend: &mut i64) {
        ...
        match self {
            ...
            RelaxationKind::GotToDirectAddressing => {
                let raw_instr = u32::from_le_bytes(
                ...
                let lui_instr = 0x37 | (rd << 7); // lui opcode with rd
                section_bytes[offset..offset + 4].copy_from_slice(&lui_instr.to_le_bytes());

                if offset + 4 < section_bytes.len() {
                    let second_instr = u32::from_le_bytes(
                        *section_bytes[offset + 4..offset + 8]
                            .first_chunk::<4>()
                            .unwrap(),
                    );

                    if (second_instr & 0x7f) == 0x03 {
                        let rs1 = (second_instr >> 15) & 0x1f;
                        let addi_instr = 0x13 | (rd << 7) | (rs1 << 15);
                        section_bytes[offset + 4..offset + 8]
                            .copy_from_slice(&addi_instr.to_le_bytes());
                    }
                }
            }
            ...
            RelaxationKind::CallToJal => {
                ...
                let jal_instr = 0x6f | (rd << 7); // jal opcode with rd
                section_bytes[offset..offset + 4].copy_from_slice(&jal_instr.to_le_bytes());
            ...
            RelaxationKind::TlsGdToIe => {
                ...
                let auipc_instr = 0x17 | (rd << 7); // auipc opcode with rd
                section_bytes[offset..offset + 4].copy_from_slice(&auipc_instr.to_le_bytes());
                ...
                if offset + 4 < section_bytes.len() {
                    ...
                    if (second_instr & 0x7f) == 0x03 {
                        section_bytes[offset + 4..offset + 8]
                            .copy_from_slice(&second_instr.to_le_bytes());
                    }
                }
            }
        }
