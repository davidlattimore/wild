...
impl<'data> crate::layout::LayoutTarget<'data> for PeLayout<'data> {
    type ArgsType = PeArgs;

    fn args(&self) -> &Args<Self::ArgsType> {
        self.args
    }

    fn layout_data(&self) -> linker_layout::Layout {
        linker_layout::Layout { files: vec![] }
    }

    fn into_target_layout(self) -> crate::layout::TargetLayout<'data> {
        crate::layout::TargetLayout::Pe(self)
    }
}
...
pub(crate) struct PeOutputSection<'data> {
    ...
    /// True if this section contains uninitialized data only (.bss).
...
pub(crate) struct SectionContribution<'data> {
    ...
    pub(crate) size: u32,
    /// The start of global symbol IDs for this object, used to map
    /// local symbol table indices to global IDs.
    pub(crate) symbol_id_start: crate::symbol_db::SymbolId,
}

// ── Section name / characteristics mapping ───────────────────────────────────

/// Determines the output section name for a given input section name.
fn output_section_name(input_name: &[u8]) -> [u8; 8] {
    // For names like ".text$foo", group under the base name
...
/// Strips alignment and link-only bits from COFF section characteristics,
/// keeping only content type and permission flags.
...
/// Extracts section alignment from COFF characteristics bits 20-23.
/// Returns alignment in bytes (1, 2, 4, 8, ..., 8192).
...
pub(crate) fn compute_layout<'data>(
...
) -> Result<crate::layout::Layout<'data, PeLayout<'data>>> {
    ...
    for group in resolved_groups {
        for file in &group.files {
            ...
            for (slot_index, slot) in resolved_obj.sections.iter().enumerate() {
                // In the PE path, sections stay as Unloaded/MustLoad (never
                // transition to Loaded because we skip the ELF layout step).
                // Include any section that wasn't discarded.
                match slot {
                ...
                // COFF section indices are 1-based.
    ...
    // Sort output sections: .text first, then .rdata, .data, .bss, then others
    ...
    for group in resolved_groups {
        for file in &group.files {
            ...
            for (local_sym_index, symbol) in object.enumerate_symbols() {
                ...
                // Only compute address if this is the canonical definition
    ...
    let entry_address = image_base + entry_point_rva as u64;
...
pub(crate) fn write<'data>(sized_output: &mut SizedOutput, layout: &crate::layout::Layout<'data, PeLayout<'data>>) -> Result {
    write_headers(&mut sized_output.out, layout)?;
    write_sections(&mut sized_output.out, layout)?;
    Ok(())
}

fn write_headers<'data>(buf: &mut [u8], layout: &crate::layout::Layout<'data, PeLayout<'data>>) -> Result {
    let e = LE;
...
fn write_sections<'data>(buf: &mut [u8], layout: &crate::layout::Layout<'data, PeLayout<'data>>) -> Result {
...
fn apply_relocations<'data>(
    buf: &mut [u8],
    layout: &crate::layout::Layout<'data, PeLayout<'data>>,
    ...
    for reloc in relocations {
        ...
        // Map local symbol table index to global symbol ID
...
/// Cast a mutable reference to a Pod type from a byte buffer at the given offset,
/// advancing the offset past the type.
fn from_bytes_mut_at<'a, T: object::pod::Pod>(buf: &'a mut [u8], offset: &mut usize) -> Result<&'a mut T> {
    ...
    // SAFETY: T is Pod, and object's PE types use byte-array fields (U16<LE>, U32<LE> etc.)
    // that don't require alignment.
