...
// ── Shared type definitions (format-agnostic) ────────────────────────────────
...
// ── End shared type definitions ──────────────────────────────────────────────

// ── Argument parser infrastructure ───────────────────────────────────────────
...
pub struct ActivatedArgs<T = TargetArgs> {
    pub args: Args<T>,
    _jobserver_tokens: Vec<jobserver::Acquired>,
}
...
mod tests {
    ...
    fn test_filter_strips_target_equals() {
        ...
        let filtered =
            filter_and_inject_target_flags(&args, OutputFormat::Elf, Some(Architecture::X86_64));
    ...
    fn test_filter_strips_target_space() {
        let args = to_strings(&["--target", "aarch64-unknown-linux-gnu", "-o", "out"]);
        let filtered =
            filter_and_inject_target_flags(&args, OutputFormat::Elf, Some(Architecture::AArch64));
        assert_eq!(filtered[0], "-m");
    ...
    fn test_filter_strips_slash_target() {
        ...
        let filtered =
            filter_and_inject_target_flags(&args, OutputFormat::Pe, Some(Architecture::X86_64));
