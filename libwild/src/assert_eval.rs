...
pub(crate) fn evaluate_assertions<'data, P: Platform>(
    processed_scripts: &[ProcessedLinkerScript<'data>],
...
fn evaluate_expression<'data, P: Platform>(
    ...
    match expr {
        ...
        Expression::Divide(l, r) => {
            let divisor = eval!(r)?;
            if divisor == 0 {
                bail!("Division by zero in linker script ASSERT expression");
            }
            Ok(eval!(l)? / divisor)
        }
        ...
        Expression::Align(expr) => {
            let align = eval!(expr)?;
            if align == 0 {
                bail!("ALIGN(0) is invalid in linker script expression");
            }
...
fn section_size<'data, P: Platform>(
    ...
    let Some(id) = output_sections.custom_name_to_id(SectionName(name)) else {
        return 0;
    };
...
fn section_address<'data, P: Platform>(
    ...
    let id = output_sections
        .custom_name_to_id(SectionName(name))
        .ok_or_else(|| {
            crate::error!(
...
mod tests {
    ...
    fn make_script(assertions: Vec<AssertCommand<'static>>) -> ProcessedLinkerScript<'static> {
    ...
    fn test_evaluate_assertions_passes() {
        let (layouts, sections) = dummy_context();
        let script = make_script(vec![AssertCommand {
        ...
        assert!(evaluate_assertions::<Elf>(&[script], &layouts, &sections).is_ok());
    }
    ...
    fn test_evaluate_assertions_fails() {
        ...
        let err = evaluate_assertions::<Elf>(&[script], &layouts, &sections).unwrap_err();
