...
pub(crate) enum Experiment {
    /// Affects the number of temporary Vecs that we'll use while doing string merging. A higher
    /// number will result in increased memory usage, increased allocator usage and possibly worse
    /// cache performance, but may result in increased parallelism.
    MergeStringCapMultiplier = 0,

    /// Approximate number of input groups per thread when doing merge string splitting.
    MergeStringGroupsPerThread = 1,

    /// If input string-merge sections are smaller than this size, we won't bother splitting them
    /// between threads.
