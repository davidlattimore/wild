...
pub struct ArgumentParser {
    ...
    short_options: std::collections::HashMap<char, OptionHandler>, // Short option lookup
...
/// Represents a prefix option handler (like -L, -l, etc.)
#[derive(Clone)]
struct PrefixOptionHandler {
    help_text: &'static str,
    handler: fn(&mut Args, &mut Vec<Modifiers>, &str) -> Result<()>,
    sub_options: std::collections::HashMap<&'static str, &'static str>, // sub-option -> help text
}
...
pub struct OptionDeclaration<'a> {
...
/// Builder for declaring prefix options (like -L, -l, etc.)
pub struct PrefixOptionDeclaration<'a> {
    parser: &'a mut ArgumentParser,
    prefix: &'static str,
    help_text: &'static str,
    sub_options: std::collections::HashMap<&'static str, &'static str>,
}

fn handle_z_option(args: &mut Args, arg: &str) -> Result<()> {
    match arg {
        "now" => {}
...
fn handle_r_option(args: &mut Args, value: &str) {
...
impl ArgumentParser {
    ...
    /// Declare an option without parameters
    pub fn declare(&mut self) -> OptionDeclaration<'_> {
        OptionDeclaration {
            parser: self,
            long_names: Vec::new(),
            short_names: Vec::new(),
    ...
    /// Declare an option with a required parameter
    pub fn declare_with_param(&mut self) -> OptionWithParamDeclaration<'_> {
        OptionWithParamDeclaration {
            parser: self,
            long_names: Vec::new(),
            short_names: Vec::new(),
            help_text: "",
    ...
    fn handle_argument<S: AsRef<str>, I: Iterator<Item = S>>(
        ...
        if let Some(path) = arg.strip_prefix('@') {
            return parse_from_argument_file(Path::new(path)).map(|_| true);
        }
        ...
        if arg.len() == 2 && arg.starts_with('-') {
            ...
            if let Some(handler) = self.short_options.get(&ch) {
                match &handler.handler {
                    OptionHandlerFn::NoParam(f) => f(args, modifier_stack)?,
                    OptionHandlerFn::WithParam(f) => {
                        let next_arg =
    ...
    pub fn generate_help(&self) -> String {
        ...
        for (prefix, handler) in prefix_options {
            ...
            if !handler.sub_options.is_empty() {
                ...
                for (sub_name, sub_help) in sub_options {
        ...
        let mut processed_short_options: std::collections::HashSet<char> =
...
impl<'a> OptionWithParamDeclaration<'a> {
    #[must_use]
    pub fn long(mut self, name: &'static str) -> Self {
        self.long_names.push(name);
        self
...
impl<'a> PrefixOptionDeclaration<'a> {
    ...
    #[must_use]
    pub fn sub_option(mut self, name: &'static str, help: &'static str) -> Self {
        self.sub_options.insert(name, help);
        self
    }
...
fn setup_argument_parser() -> ArgumentParser {
    ...
    parser
        ...
        .sub_option(":filename", "Link with specific file")
        .sub_option("libname", "Link with library libname.so or libname.a")
    ...
    parser
        ...
        .sub_option("elf_x86_64", "x86-64 ELF target")
        .sub_option("aarch64elf", "AArch64 ELF target")
        .sub_option("elf64lriscv", "RISC-V 64-bit ELF target")
        .execute(|args, _modifier_stack, value| {
    ...
    parser
        .declare_prefix("z")
        .help("Linker option")
        .sub_option("now", "Set BIND_NOW for object")
        .sub_option("origin", "Enable origin handling")
        .sub_option("relro", "Enable relocation read-only")
        .sub_option("norelro", "Disable relocation read-only")
        .sub_option("notext", "Allow relocations in read-only sections")
        .sub_option("nostart-stop-gc", "Disable start/stop symbol GC")
        .sub_option("execstack", "Enable executable stack")
        .sub_option("noexecstack", "Disable executable stack")
        .sub_option("nocopyreloc", "Disable copy relocations")
        .sub_option("nodelete", "Mark object as non-deletable")
        .sub_option("defs", "Report unresolved symbol references")
        .sub_option("muldefs", "Allow multiple definitions")
        .sub_option("lazy", "Use lazy binding (default)")
        .execute(|args, _modifier_stack, value| {
            handle_z_option(args, value)?;
            Ok(())
        });
    ...
    parser
        .declare()
        .long("gc-sections")
        .help("Enable garbage collection of unused sections")
        .execute(|args, _modifier_stack| {
    ...
    parser
        .declare()
        .long("no-gc-sections")
        .help("Disable garbage collection of unused sections")
        .execute(|args, _modifier_stack| {
    ...
    parser
        .declare()
        .long("no-pie")
        .help("Create a position-dependent executable")
        .execute(|args, _modifier_stack| {
    ...
    parser
        ...
        .help("Do not use a dynamic linker")
        .execute(|args, _modifier_stack| {
    ...
    parser
        .declare()
        .long("as-needed")
        .help("Only link shared libraries if needed")
        .execute(|_args, modifier_stack| {
    ...
    parser
        .declare()
        .long("no-as-needed")
        .help("Always link shared libraries")
        .execute(|_args, modifier_stack| {
    ...
    parser
        ...
        .help("Only include needed objects from archives")
        .execute(|_args, modifier_stack| {
    ...
    parser
        .declare()
        .long("push-state")
        .help("Push current linker state")
        .execute(|_args, modifier_stack| {
    ...
    parser
        .declare()
        .long("no-undefined")
        .help("Report unresolved symbol references")
        .execute(|args, _modifier_stack| {
    ...
    parser
        ...
        .help("Handle unresolved symbols")
        .execute(|args, _modifier_stack, value| {
    ...
    parser
        .declare_with_param()
        .long("rpath-link")
        .help("Runtime library search path")
        .execute(|_args, _modifier_stack, _value| {
            // TODO
            Ok(())
        });
    ...
    parser
        ...
        .help("Allow undefined symbols in shared libraries")
        .execute(|args, _modifier_stack| {
    ...
    parser
        ...
        .help("Disallow undefined symbols in shared libraries")
        .execute(|args, _modifier_stack| {
    ...
    parser
        ...
        .help("Error on unresolved symbols")
        .execute(|args, _modifier_stack| {
    ...
    parser
        ...
        .help("Warn on unresolved symbols")
        .execute(|args, _modifier_stack| {
...
fn add_ignored_flags(parser: &mut ArgumentParser) {
