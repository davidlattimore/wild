...
pub(crate) fn parse<F: Fn() -> I, S: AsRef<str>, I: Iterator<Item = S>>(input: F) -> Result<Args> {
    ...
    let should_set_no_undefined = match args.output_kind() {
        OutputKind::SharedObject => {
            matches!(
                args.unresolved_symbols,
                UnresolvedSymbols::IgnoreInObjectFiles
            )
        }
        _ => {
            matches!(
                args.unresolved_symbols,
                UnresolvedSymbols::IgnoreInSharedLibs | UnresolvedSymbols::ReportAll
            )
        }
    };

    if should_set_no_undefined {
        args.no_undefined = true;
    }
