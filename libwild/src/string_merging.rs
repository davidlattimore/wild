...
const MIN_GROUP_BYTES: u64 = 1024;
...
pub(crate) fn merge_strings<'data>(
    ...
    let reuse_pool = ReusePool::new(
        MERGE_STRING_BUCKETS
            * args.numeric_experiment(Experiment::MergeStringCapMultiplier, 80) as usize
            / 10,
    );
...
fn create_split_resources<'data, 'offsets, 'scope>(
    ...
    let groups_per_thread = args.numeric_experiment(
        Experiment::MergeStringGroupsPerThread,
        APPROXIMATE_GROUPS_PER_THREAD as u64,
    ) as usize;
    ...
    let target_group_size =
        min_group_bytes.max((input_size.0 as usize / num_threads / groups_per_thread) as u64);

    let unprocessed: ArrayQueue<SectionGroup> = ArrayQueue::new(groups_per_thread * num_threads);
