...
pub struct Linker {
    ...
    linker_plugin_arena: Arena<linker_plugins::LinkerPluginStore>,
...
impl Linker {
    ...
    fn load_inputs_and_link<'data, A: arch::Arch>(
        ...
        if let Some(plugin) = plugin.as_mut() {
            let plugin_outputs = plugin.all_symbols_read(&symbol_db, &resolver.resolved_groups)?;

            let plugin_loaded =
                file_loader.load_inputs(&plugin_outputs.generated_inputs, args, &mut None)?;
