//! ELF-specific layout computation. Traverses the graph of symbol references to figure out what
//! sections from the input files are referenced. Determines which sections need to be linked, sums
//! their sizes, decides what goes where in the output file, then allocates addresses for each symbol.
...
use crate::layout::SectionResolution;
...
pub fn compute<'data, P: ElfPlatform<'data>>(
...
) -> Result<crate::layout::Layout<'data, ElfLayout<'data>>> {
    timing_phase!("Layout");
    ...
    let file_size = compute_total_file_size(&section_layouts);
    output.set_size(file_size);
    ...
    // Build the shared CommonLayout from ELF-specific data.
    let symbol_addresses: Vec<u64> = symbol_resolutions
        .resolutions
        .iter()
        .map(|r| r.map_or(0, |r| r.raw_value))
        .collect();

    Ok(crate::layout::Layout::new(
        file_size,
        0, // entry_address filled in lazily via entry_symbol_address()
        symbol_addresses,
        ElfLayout {
            symbol_db,
            symbol_resolutions,
...
/// ELF-specific layout data. Wrapped in `crate::layout::Layout<ElfLayout>` which provides the
/// common fields (file_size, entry_address, symbol_addresses).
#[derive(Debug)]
pub(crate) struct ElfLayout<'data> {
...
impl<'data> crate::layout::LayoutTarget<'data> for ElfLayout<'data> {
    ...
    fn into_target_layout(self) -> crate::layout::TargetLayout<'data> {
        crate::layout::TargetLayout::Elf(self)
    }
}
...
/// the next iteration.
fn relaxation_scan_pass<'data, P: ElfPlatform<'data>>(
    group_states: &mut [GroupState<'data>],
    section_part_layouts: &OutputSectionPartMap<OutputRecordLayout>,
...
fn perform_iterative_relaxation<'data, P: ElfPlatform<'data>>(
    group_states: &mut [GroupState<'data>],
    section_part_sizes: &mut OutputSectionPartMap<u64>,
