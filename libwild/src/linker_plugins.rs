...
pub(crate) struct LinkerPlugin<'data> {
    store: &'data mut LinkerPluginStore,
    herd: &'data Herd,
...
/// unloaded.
pub(crate) struct LinkerPluginStore {
    callbacks: Callbacks,
...
impl<'data> LinkerPlugin<'data> {
    pub(crate) fn from_args(
        args: &crate::Args,
        arena: &'data Arena<LinkerPluginStore>,
        herd: &'data Herd,
    ) -> Result<Option<LinkerPlugin<'data>>> {
        match args.plugin_path.as_ref() {
            Some(path) => {
                let plugin = arena.alloc(
                    LinkerPluginStore::new(Path::new(path), args)
                        .with_context(|| format!("Failed to initialise linker plugin `{path}`"))?,
                );
    ...
    pub(crate) fn all_symbols_read(
        ...
        if !has_loaded_lto_input(resolved_groups) {
            return Ok(PluginOutputs::default());
        }

        timing_phase!("Linker plugin codegen");

        self.store.with_callbacks(|callbacks| {
...
impl LinkerPluginStore {
    fn new(plugin_path: &Path, args: &Args) -> Result<LinkerPluginStore> {
...
impl Drop for LinkerPluginStore {
    fn drop(&mut self) {
...
impl std::fmt::Display for LinkerPlugin<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(version_info) = self.store.version_info.as_ref() {
