...
pub(crate) trait ObjectFile<'data>: Send + Sync + Sized + std::fmt::Debug + 'data {
    ...
    /// Whether to emit GOT/PLT symbol table entries. ELF-specific, returns false by default.
    fn wants_got_plt_syms(args: &Args<Self::ArgsType>) -> bool {
        let _ = args;
        false
    }

    /// Stack size for the stack segment. Returns 0 if unset. ELF-specific.
    fn stack_size(args: &Args<Self::ArgsType>) -> u64 {
        let _ = args;
        0
    }

    /// Whether RELRO is enabled. ELF-specific, returns false by default.
    fn wants_relro(args: &Args<Self::ArgsType>) -> bool {
        let _ = args;
        false
    }

    /// Whether SysV hash is included in hash style. ELF-specific, returns false by default.
    fn hash_includes_sysv(args: &Args<Self::ArgsType>) -> bool {
        let _ = args;
        false
    }
}
