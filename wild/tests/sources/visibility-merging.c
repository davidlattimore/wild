...
// This symbol is exported, but is protected, since the definition in the
// second file is.
...
// Note, we don't check direct references to `data2` and `data3`. In the case
// of `data2`, direct references wouldn't be permitted, since the symbol is
// interposable. In the case of `data3`, the symbol will be protected, so
// direct references should be permitted, however GNU ld < 2.40 would error in
// the case of direct references to protected symbols, so in order to allow our
// tests to pass with such versions, we don't.
