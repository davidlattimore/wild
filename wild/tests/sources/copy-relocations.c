...
// These two symbols are at the same address in the shared object, so
// references to both should point to the same copy relocation and that
// location should be what `get_w` returns.
extern int w1;
extern int s1;
int get_w1 (void);
int get_s1 (void);
