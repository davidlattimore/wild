// This test links against libc and checks that various things work as
// expected.
...
// Each binary links against shared objects created by that linker. So
// different names are expected.
...
// initial-exec rather than local exec. lld does however still optimise to
// local exec, so we enable it as an additional reference point.
//#EnableLinker:lld
...
main ()
{
  ...
  if (compute_value10 () != 10)
    ...
    // override ours. However if we're statically linking our dependency then
    // its strong definition will override ours.
#ifdef DYNAMIC_DEP
  int expected = 7;
#else
  int expected = 42;
  ...
  if (&old_timer_gettime == NULL)
    ...
    // We have a weak, hidden definition of atoi in libc-integration-0.c.
    // Provided we're dynamically linking, that definition shouldn't be
    // exported from the shared object on account if it being hidden. That
    // means that we should get the proper definition from libc.
#ifdef DYNAMIC_DEP
  if (atoi ("1000") != 1000)
    {
      return 126;
