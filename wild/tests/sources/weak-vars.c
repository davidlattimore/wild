...
_start ()
{
  ...
  if (&weak_var3)
    {
      // This is different in that it doesn't reference the weak variable in
      // the block, which means it optimises differently.
      value += 32;
    }
  if (weak_arr1)
    {
  ...
  if (weak_arr2)
    ...
    // Referencing a strong variable that's defined in the same object as our
    // weak symbols can affect what definition gets used. We have two strong
    // variables, one in DATA, the same as our weak variables, the other in
    // BSS. We only ever want one or the other, but we also want to try
