...
black_box (int input)
{
  ...
  __asm__ __volatile__ ("nop" : "+r"(rdi));
  return rdi;
}
...
_start ()
{
  ...
  // Without passing our value through a black box, the compiler gets rid of
  // the if-statement below, treating it as always true, since it figures that
  // an integer obtained from a pointer can never be equal to 25.
